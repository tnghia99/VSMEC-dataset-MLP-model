...
class ModelTraining(object):
    ...
    def train_model(self):
        ...
        tfidf.fit_transform(data.Sentence)
        ...
        model = MLPClassifier(random_state=0, hidden_layer_sizes=35, alpha=0.54, verbose=True, max_iter=600)
    ...
    def train_model_without_other(self):
        ...
        x_train = tfidf.transform(data.Sentence)
        y_train = data.Emotion
        model = MLPClassifier(random_state=0, hidden_layer_sizes=50, alpha=0.5, verbose=True, max_iter=600)
        model.fit(x_train, y_train)
        # save MLP trained model
